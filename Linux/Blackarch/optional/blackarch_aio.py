        ...
        try:
            result = subprocess.run(
                [
                    "bash",
                    "-c",
                    "LC_ALL=C pacman -Qkk 2>&1 | grep 'missing dependency' | awk -F'\"' '{print $2}' | sort -u",
                ],
                capture_output=True,
                text=True,
                timeout=120,
            ...
            if not broken_pkgs:
                ...
                return
            logging.info(
                f"Found {len(broken_pkgs)} packages with broken .so deps: {broken_pkgs}"
            )
    ...
    def cleanup_stale_python_packages():
        ...
        try:
            ...
            if not pkgs_to_remove:
                logging.debug(
                    "All Python packages have current interpreter files — nothing to remove."
                )
                return

            logging.info(
                f"Found {len(pkgs_to_remove)} stale Python packages (no {current_pyver} files): "
