...
class FastUpdate:
    ...
    async def _pin_downgrade_conflicts(self):
        ...
        for broken_dep, parent_pkg in conflicts:
            logging.info(
                f"Downgrade conflict: '{parent_pkg}' needs '{broken_dep}' (missing) — pinning"
            )
        ...
        logging.info(
            f"Pinned packages (immune to -Syuu downgrade): {self.ignore_pkgs}")
