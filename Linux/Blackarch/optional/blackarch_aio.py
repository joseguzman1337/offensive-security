...
class FastUpdate:
    ...
    async def download_phase(self):
        ...
        tasks.append(
            self.run_command(
                " ".join(AUR_HELPERS["pacman"]["download"]),
                "Downloading Pacman updates",
            )
        )
    ...
    async def install_phase(self):
        ...
        # 1. System upgrade
        success, err = await self.run_command(
            " ".join(AUR_HELPERS["pacman"]["upgrade"]),
            "Installing Pacman updates",
            silent=False,
        )
        if not success:
            return False, err
    ...
    async def execute(self):
        ...
        steps = [
            ...
            (
                "Granular Package Upgrade",
                lambda: asyncio.to_thread(
                    PackageManager.upgrade_all_packages_synced),
            ),
            (
                "Cleanup Orphans",
                lambda: self.run_command(
